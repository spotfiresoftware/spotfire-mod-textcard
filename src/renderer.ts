...
import { DataViewHierarchy, DataViewRow, MarkingOperation } from "spotfire-api";
import { createTextCard } from "./creator";
import { configureMouseOver, getSelectedText, isAllRowsMarked } from "./helper";
 ...
 */
export function renderTextCards(
  rows: DataViewRow[],
  prevIndex: number,
  cardsToLoad: number,
  rerender: boolean,
  ...
  mod: Spotfire.Mod,
  hierarchy: { tooltip: DataViewHierarchy; annotation: DataViewHierarchy }
) {
  if (rerender) {
  ...
  let allRowsMarked = isAllRowsMarked(rows);
  ...
  for (let index = startIndex; index < whatToLoad; index++) {
    ...
    if (rows[index].categorical("Content").value()[0].key) {
      ...
      let cardMaxHeight =
        rows.length == 1
          ? windowSize.height - paddingHeight
          : windowSize.height / 2;
      ...
      newDiv.onmousedown = (event) => {
        if (event.shiftKey) {
          document.getElementById("text-card-container")!.style.userSelect =
            "none";
        }
        var scrolling = true;
        ...
        newDiv.onmouseup = function () {
          if (!scrolling) {
            ...
            let operation: MarkingOperation = event.ctrlKey
              ? "ToggleOrAdd"
              : "Replace";
            if (!event.shiftKey) {
              ...
              if (lastMarkedIndex > index) {
                for (var ii = index; ii <= lastMarkedIndex; ii++) {
                  rows[ii].mark(operation);
                }
              }
            }
          }
