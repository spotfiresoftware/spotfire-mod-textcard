...
import { clearWarning, createSortButton, createWarning } from "./creator";
import {
  findDivElem as findDivElem,
  getSelectedText,
  sortRows,
  textToClipboard,
} from "./helper";
import { renderTextCards } from "./renderer";
...
Spotfire.initialize(async (mod) => {
  ...
  const modDiv = findDivElem("#text-card-container");
  ...
  async function render(
    ...
    contentAxis: Spotfire.Axis,
    sortAxis: Spotfire.Axis,
    sortOrder: ModProperty<string>,
    cardByAxis: Spotfire.Axis,
    useCustomCardBy: ModProperty<boolean>
  ) {
    // Check card by axis expression.
    if (cardByAxis.expression !== "<baserowid()>" && !useCustomCardBy.value()) {
      createWarning(
        modDiv,
        context.styling.general.font.color,
        cardByAxis,
        useCustomCardBy
      );
    ...
    if (contentAxis.parts.length == 0) {
      mod.controls.errorOverlay.show(
        "Select the 'Content' of the text cards to get started!"
      );
      return;
    } else if (contentAxis.parts.length > 1) {
      if (contentAxis.parts.length > 1) {
        mod.controls.errorOverlay.show(
          "Selecting multiple columns in 'Content' is not supported."
        );
      } else {
        mod.controls.errorOverlay.show(
          "Something went wrong. Please reload the mod."
        );
      }
      return;
    }
    ...
    let sortingEnabled =
      sortAxis.expression != "" && sortAxis.expression != "<>";
    if (sortingEnabled) {
      // create sort button only if there is a value selected in sorting axis
      sortRows(rows, sortOrder.value() ?? "asc", sortAxis.isCategorical);
    }
    ...
    modDiv.onmousedown = (e: MouseEvent) => {
    ...
    if (sortingEnabled)
      createSortButton(
        modDiv,
        mod.getRenderContext().styling.general.font.color,
        sortOrder
      );
