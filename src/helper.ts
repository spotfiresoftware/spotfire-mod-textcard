import { DataViewHierarchy, DataViewRow } from "spotfire-api";
import { createCopyButton, createTooltipString } from "./creator";
 ...
 */
export function truncateString(dataValue: string, maxLength: number) {
  // Slice at maxLength minus 3 to really return maxLength characters
  return dataValue.slice(0, maxLength - 3) + "...";
}

type CardDiv = {
  textCardDiv: HTMLDivElement;
  header: HTMLDivElement | undefined;
  content: HTMLDivElement | undefined;
};

type CardHierarchyInfo = {
  tooltip: DataViewHierarchy;
  annotation: DataViewHierarchy;
};
/**
 ...
 */
export function configureMouseOver(
  divObject: CardDiv,
  borderDiv: HTMLDivElement,
  fontStyling: Styling,
  row: DataViewRow,
  mod: Spotfire.Mod,
  hierarchy: CardHierarchyInfo
) {
  ...
  if (!hierarchy.tooltip.isEmpty && divObject.content) {
  ...
  if (!hierarchy.annotation.isEmpty && divObject.header) {
 ...
 */
export function isAllRowsMarked(rows: DataViewRow[]) {
...
export function sortRows(
  ...
  rows.sort(function (row1, row2) {
    if (categoricalSorting) {
      ...
      return (
        order *
        (row1.categorical("Sorting").leafIndex -
          row2.categorical("Sorting").leafIndex)
      );
    }
 ...
 */
export function textToClipboard(text: string | null) {
  if (!text) {
    return;
  }
 ...
 */
export function findDivElem(selector: string) {
