...
import { findDivElem, textToClipboard, truncateString } from "./helper";
import { Styling } from "./renderer";
...
export function createTextCard(
  content: string,
  annotation: DataViewCategoricalValuePathElement[] | null,
  maxHeight: number,
  markObject: { row: boolean; allRows: boolean },
  fontStyling: Styling,
  lineDividerColor: string
) {
  // create div
  ...
  let contentParagraph: HTMLDivElement | undefined;
  // add paragraph to text card
  if (typeof content === "string") {
    contentParagraph = createTextCardContentParagraph(
      maxHeight,
      content,
      fontStyling
    );
 ...
 * @returns {string}
 */
export function createTooltipString(
  specificRow: DataViewRow,
  tooltipHierarchy: DataViewHierarchy
) {
  var nrOfTooltipChoices = specificRow
    .categorical(tooltipHierarchy.name)
    .value().length;
  ...
  for (i = 0; i < nrOfTooltipChoices; i++) {
    ...
    if (dataValue !== null) {
      ...
      tooltipString =
        tooltipString +
        tooltipObj.columnName +
        ": " +
        tooltipObj.dataValue +
        "\n";
    }
  }
 ...
 */
export function createTextCardContentParagraph(
  maxHeight: number,
  content: string,
  fontStyling: Styling
) {
 ...
 * @returns {HTMLDivElement}
 */
export function createHeaderContent(annotation: string) {
 ...
 */
export function createLineDividerInTextCard(lineColor: string) {
 ...
 * @returns {HTMLDivElement}
 */
export function createTextCardDiv(fontStyling: Styling) {
 ...
 * @returns {HTMLDivElement}
 */
export function createTextCardHeader() {
 ...
 */
export function createCopyButton(newDiv: HTMLDivElement, buttonColor: string) {
  // create element
  ...
  svg.setAttributeNS(
    null,
    "d",
    "M11.259 1H6v3H2v11h10v-3h2V4.094zM8 4h2v1H8zm3 10H3V5h3v7h5zm1-5H8V8h4zm0-2H8V6h4z"
  );
...
export function createWarning(
  modDiv: HTMLDivElement,
  textColor: string,
  cardbyProp: Axis,
  customExpression: ModProperty<boolean>
) {
  // get warning div
  var warningDiv = findDivElem("#warning-message") as HTMLDivElement;
  ...
  if (
    cardbyProp.expression !== "<>" &&
    cardbyProp.expression !== "<baserowid()>"
  ) {
    ignoreButton.setAttribute(
      "class",
      "spotfire-button-flex spotfire-button-white"
    );
  ...
  resetButton.setAttribute(
    "class",
    "spotfire-button-flex spotfire-button-blue"
  );
